...
case 'song': {
    ...
    // ✨ Compress Function 
    async function compressAudio(input, output) {
        try {
            ...
            const bitrate = Math.min(Math.max(Math.floor((TARGET_MB * 8192) / duration), 32), 128);

            await execPromise(`ffmpeg -i "${input}" -b:a ${bitrate}k -y "${output}"`);
            return true;
        } catch {
            return false;
        }
    ...
    if (!q.trim())
        return socket.sendMessage(sender, { text: "*🎵 Give me a song title or YouTube link!*" });

    // ✨ Search video (GiftedTech API)
    const searchUrl = `https://api.giftedtech.web.id/api/search/ytsearch?apikey=gifted&query=${encodeURIComponent(q)}`;
    const searchData = await fetch(searchUrl).then(r => r.json());

    if (!searchData?.result?.[0])
        return socket.sendMessage(sender, { text: "*❌ No results found!*" });

    const vid = searchData.result[0];
    ...
    try {
        // ✨ Download MP3 (Gifted API)
        const apiUrl = `https://api.giftedtech.web.id/api/download/dlmp3?apikey=gifted&url=${encodeURIComponent(videoUrl)}`;
        const mp3Data = await fetch(apiUrl).then(r => r.json());

        if (!mp3Data?.downloadUrl)
            return socket.sendMessage(sender, { text: "*❌ Download failed!*" });

        // ✨ Download file to temp
        const cleanTitle = vid.title.replace(/[^\w\s]/gi, "").substring(0, 20);
        let original = path.join(TEMP_DIR, `${cleanTitle}_org.mp3`);
        let final = path.join(TEMP_DIR, `${cleanTitle}_final.mp3`);

        const buffer = await fetch(mp3Data.downloadUrl).then(r => r.arrayBuffer());
        await fs.writeFile(original, Buffer.from(buffer));

        // ✨ Check file size
        const stats = await fs.stat(original);
        const sizeMB = stats.size / (1024 * 1024);

        if (sizeMB > MAX_SIZE_MB) {
            const ok = await compressAudio(original, final);
            if (ok) {
        ...
        console.log(err);
        socket.sendMessage(sender, { text: "*❌ Error downloading song!*" });
    }
