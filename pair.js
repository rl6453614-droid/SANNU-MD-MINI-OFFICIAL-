...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    IMAGE_PATH: 'https://files.catbox.moe/vrahf4.jpg',
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/LcOBCsUP0wl8xSDnngXzjp?mode=wwt',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/vrahf4.jpg',
    NEWSLETTER_JID: '120363282833839832@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '94772563976',
    BOT_FOOTER: '>  © 𝙿𝙾𝚆𝙴𝚁𝙳 𝙱𝚈🥷𝐒𝐀𝐍𝐍𝐔-𝐌𝐃',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb6szVQCsU9UjD976t14'
};

const octokit = new Octokit({ auth: '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' });
const owner = 'rl6453614-droid';
const repo = 'SANNU-XD-MINI';
...
`;
        const aliveMessage = {
            image: { url: "https://files.catbox.moe/vrahf4.jpg" },
            caption: `> © 𝙿𝙾𝚆𝙴𝚁𝙳 𝙱𝚈🥷𝐒𝐀𝐍𝐍𝐔-𝐌𝐃\n\n${captionText}`,
            buttons: [
                {
...
┗☐━❲🔸𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃🔸❳━☐`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: '>  © 𝙿𝙾𝚆𝙴𝚁𝙳 𝙱𝚈🥷𝐒𝐀𝐍𝐍𝐔-𝐌𝐃',
                serverMessageId: -1
            }
        };
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: 'ᴍɪɴɪ 𝚂𝙰𝙽𝙽𝚄 𝚖𝚍 ᴄᴍᴅs' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              title: 'ᴍɪɴɪ 𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 ᴄᴍᴅs',
              sections: [
                {
                  title: "🌐 ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs",
                  highlight_label: 'ᴍɪɴɪ 𝚂𝙰𝙽𝙽𝚄 𝙼𝙳',
                  rows: [
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363282833839832@newsletter'
                        });
                    }
...
case 'song': {
    ...
    const axios = require('axios'); // To fetch API data
    const fs = require('fs');

    // --- Configuration ---
    // ඔබට කැමති API එකක් මෙතනට භාවිතා කළ හැක. 
    // උදාහරණයක් ලෙස අපි පොදු API රටාවක් භාවිතා කරමු.
    // විශ්වාසවන්ත API එකක් සොයාගැනීම වැදගත් (Ex: cobalt, dark-ytdl, etc.)
    ...
    try {
        // 1. Search for the video using yt-search
        ...
        // 2. Send Info Message (Thumbnail & Details)
        const desc = `
     ᴍɪɴɪ 𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳
╭──────────────────────⭓
...
`;
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363193438966666@newsletter', // ඔබේ නාලිකාවේ ID එක දමන්න
                    newsletterName: '𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙸𝙽𝙸',
                    serverMessageId: -1
                }
            }
        }, { quoted: fakevCard });

        // 3. Fetch Audio URL from API
        // NOTE: මෙතැනට හොඳ API එකක් භාවිතා කරන්න. 
        // උදාහරණයක් ලෙස: https://api.giftedtech.my.id/api/download/dlmp3?url=
        // නැතහොත් ඔබ දන්නා වෙනත් API එකක්.
        
        const apiUrl = `https://api.dreaded.site/api/yimp3?url=${videoInfo.url}`; 
        // (වෙනත් API options: 'https://api.giftedtech.my.id/api/download/dlmp3?url=')

        const response = await axios.get(apiUrl);
        
        // API එක අනුව response structure එක වෙනස් විය හැක.
        // බොහෝ විට response.data.result.downloadUrl හෝ response.data.result වැනි තැනක URL එක තිබේ.
        const downloadUrl = response.data.result?.download?.url || response.data.result?.url || response.data.url;

        if (!downloadUrl) {
            throw new Error("API didn't return a download URL");
        }

        // 4. Send Audio
        await socket.sendMessage(sender, {
            audio: { url: downloadUrl }, // කෙලින්ම URL එක හරහා යැවීම (Buffer අවශ්‍ය නැත)
            mimetype: "audio/mpeg",
            fileName: `${videoInfo.title}.mp3`,
            ptt: false // Voice note ලෙස යැවීමට අවශ්‍ය නම් true කරන්න
        }, { quoted: fakevCard });
...
`;
    ...
    await socket.sendMessage(sender, {
      image: { url: thumbnail || 'https://files.catbox.moe/vrahf4.jpg' }, // Fallback image
      caption
    }, { quoted: fakevCard });
    ...
    break;
                ...
                case 'winfo': {
                    ...
                    try {
                        ...
                        winfoPpUrl = 'https://files.catbox.moe/vrahf4.jpg';
                    }
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is 𝚂𝙰𝙽𝙽𝚄🥷🍂. 
...
If asked "Who are you?": Reply "I'm 𝚂𝙰𝙽𝙽𝚄 𝙼𝙳👾".  
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/vrahf4.jpg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
 ...
 // Case: add - Add a member to the group
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/vrahf4.jpg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
            ...
            try {
                if (hasImage) {
                    await socket.sendMessage(group.id, {
                        ...
                        caption: broadcastMessage ? `┏☐━❲🔸𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃🔸❳━☐\n│\n│ 📢 *Broadcast*\n│\n│ ${broadcastMessage}\n│\n┗☐━❲🔸𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃🔸❳━☐\n> 𝚂𝙰𝙽𝙽𝚄-𝙼𝙳 🍂` : undefined
                    });
                } else if (hasVideo) {
                    await socket.sendMessage(group.id, {
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/rl6453614-droid/raganork-md';
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363282833839832@newsletter',
                    newsletterName: '𝚂𝙰𝙽𝙽𝚄-𝙼𝙳 🥷',
                    serverMessageId: 143
                }
            }
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/rl6453614-droid/raganork-md`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/rl6453614-droid/raganork-md',
                sourceUrl: 'https://github.com/rl6453614-droid/raganork-md'
            }
        }
    }, { quoted: fakevCard });
    break;
    ...
    await socket.sendMessage(sender, {
        ...
        contextInfo: {
            externalAdReply: {
                ...
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/rl6453614-droid',
                sourceUrl: 'https://github.com/rl6453614-droid'
            }
        }
    }, { quoted: fakevCard });
    break;
...
>  © 𝙿𝙾𝚆𝙴𝚁𝙳 𝙱𝚈🥷𝐒𝐀𝐍𝐍𝐔-𝐌𝐃
  ...
  𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 වෙතට සාදරයෙන් පිලිගන්නවා🍂𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝚂𝙰𝙽𝙽𝚄 𝙼𝙳 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙴𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃👋`
});
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || '𝚂𝙰𝙽𝙽𝚄-𝙼𝙳 𝚖𝚊𝚒𝚗'}`);
                }
            }
        });
    } catch (error) {
